      ...
      case "forget_memory": {
        ...
        if (resolution.status === "none") {
          ...
          result = {
            ...
            message:
              "No remembered fact matches that. NOTHING was deleted. Do NOT retry with reworded guesses. If the user was trying to change YOUR name or personality, stop using this tool entirely — that is not a stored memory and never will be; tell them to set the \"Custom system prompt\" field under Settings → Devices & services → Nives → Configure, writing just the personality they want. If you already previewed this exact memory earlier in this conversation, it is simply GONE ALREADY — tell the user it is no longer stored, which is what they asked for; do NOT report a failure or say you couldn't find it. Otherwise: if exactly one of 'suggestions' is clearly the memory the user means, call forget_memory once more with that suggestion's exact text (it will still only preview); if none is, tell the user you don't have that memory — never delete something merely similar.",
          };
          break;
        }
